...
plotCircular <- function( s1, s2, s1.chr, s2.chr, ortho.t, chr.sep=1e7, main='', label.depth=4,
    ...
    plot(1,1, type='n', axes=FALSE, xlim=c(-110, 110), ylim=c(-110, 110), xlab='', ylab='',
         main=main, asp=1)
...
colnames(mRNA)  <- c('mRNA_id', 'chr', 'strand', 'start', 'end', 'v6', 'id', 'v8', 'v9')
...
stickleback.db <- dbConnect(MySQL(), dbname='gasterosteus_aculeatus_core_89_1')
